...
CREATE TABLE IF NOT EXISTS my_scenes_params (
                ...
                ) WITH "template=MyMeta_template,affinityKey=scenes_name,KEY_TYPE=cn.plus.model.db.MyScenesParamsPk,VALUE_TYPE=cn.plus.model.db.MyScenesParams,cache_name=my_scenes_params,ATOMICITY=TRANSACTIONAL_SNAPSHOT,cache_group=my_meta";
CREATE INDEX IF NOT EXISTS scenes_params_idx ON my_scenes_params (scenes_name, group_id);

CREATE INDEX IF NOT EXISTS scenes_obj_tn_idx ON my_scenes (scenes_name, table_name);
CREATE INDEX IF NOT EXISTS scenes_obj_in_idx ON my_scenes (scenes_name, index_name);
